 ...
 *
 * 
 * OWL FUCKED AROUND WITH THIS TO ADD THE GROUP CHECK. IF THIS EVER BREAKS,
 * DO NOT BOTHER ME BY EMAILING <owlmanatt@gmail.com> BECAUSE I PROBABLY
 * DON'T FUCKING CARE.
 *
 * PATCHING INSTRUCTIONS: DROP THIS IN authentication/phpbb3 
 *
 ***************************************************************************/
...
function pVerify($user) {
    ...
    // Also make sure it's ACTUALLY A FUCKING GOON. I assume you set up a user group
    // and put people in it once you've validated them.
    $AUTH_GROUP_ID = 8; // this is my 'Validated Faggots' group ID.
    $goon = (bool)group_memberships($AUTH_GROUP_ID,$user->data['user_id']);

	if ($user->data['user_id'] != '1' && $user->data['is_registered'] == 1 && $user->data['username'] != 'Anonymous' && empty($user->data['is_bot']) && $goon == true) {
		$sql['SELECT'] = '*';
		$sql['FROM'] = 'profile';
...
if( isset( $_GET["phpbb_root_path"] ) || isset( $_POST["phpbb_root_path"]) || isset($_GET['pConfig_auth[phpbb_path]']) || isset($_POST['pConfig_auth[phpbb_path]']) ) {
    ...
    // Also include this for the group_memberships() function. For some fucking reason,
    // common.php doesn't include this.
	include_once($pConfig_auth['phpbb_path'].'/includes/functions_user.php');
